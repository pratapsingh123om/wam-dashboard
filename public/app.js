/* public/app.js — WAM Dashboard frontend (complete replacement)
   Key changes:
    - Configurable API_BASE via window.WAM_API_BASE
    - Robust SSE (single EventSource + exponential backoff reconnect)
    - Chart update scheduling retained, charts created once
    - No repeated startSSE via setInterval (avoids duplicate connections)
*/
...
(() => {
  ...
  // Config: set window.WAM_API_BASE = "http://localhost:5001" if your backend is separate.
  ...
  const alertsBar = $('alerts') || (() => { const f=document.createElement('div'); f.id='alerts'; (q('.footer')||document.body).appendChild(f); return f; })();
  ...
  // thresholds and manual inputs fallback
  ...
  function readThresholds(){ return { phLow: parseFloat(thPhLow.value)||6.5, phHigh: parseFloat(thPhHigh.value)||8.5, tds: parseFloat(thTds.value)||500, turb: parseFloat(thTurb.value)||5, iron: parseFloat(thIron.value)||0.3 }; }
  function showStatus(msg, type='info'){ if(!alertsBar) return; alertsBar.textContent = msg; alertsBar.style.background = type==='ok'? 'linear-gradient(90deg, rgba(0,255,170,0.04), rgba(0,255,170,0.01))' : (type==='warn'? 'linear-gradient(90deg, rgba(255,200,80,0.04), rgba(255,120,0,0.01))' : 'transparent'); }
  ...
  function destroyAllCharts(){
    destroyChartInstance(mainChart); destroyChartInstance(donutPH); destroyChartInstance(donutTDS); destroyChartInstance(donutTURB);
    Object.values(modelCharts).forEach(destroyChartInstance);
    mainChart=donutPH=donutTDS=donutTURB=null; modelCharts={};
  }

  function createDonut(canvas){
    const ctx=canvas.getContext('2d');
    return new Chart(ctx, { type:'doughnut', data:{ labels:['Low','Mid','High'], datasets:[{ data:[0,0,0], backgroundColor:['#62d0b3','#8fa6b8','#d68cff'] }] }, options:{ plugins:{ legend:{ display:false } }, maintainAspectRatio:false } });
  }
    ...
    if(!mainChart){
      ...
      mainChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            { label:'pH', data: chartSeries.ph, borderColor:'#62d0b3', tension:0.25, spanGaps:true, yAxisID:'y1' },
            { label:'TDS', data: chartSeries.tds, borderColor:'#7cb7ff', tension:0.25, spanGaps:true, yAxisID:'y2' },
            { label:'Turb', data: chartSeries.turb, borderColor:'#ffd166', tension:0.25, spanGaps:true, yAxisID:'y2' },
            { label:'Iron', data: chartSeries.iron, borderColor:'#d68cff', tension:0.25, spanGaps:true, yAxisID:'y2' }
          ]
        },
        options: {
          animation:false,
          maintainAspectRatio:false,
          scales:{ x:{ ticks:{ maxRotation:0 } }, y1:{ position:'left', title:{display:true,text:'pH'} }, y2:{ position:'right', grid:{ drawOnChartArea:false }, title:{display:true,text:'mg/L / NTU'} } },
  ...
  // detection + prediction (kept unchanged)
  function evaluateLatest(){
    const n = chartSeries.ts.length;
    if(n===0){ detectedText && (detectedText.textContent='No data yet'); treatmentText && (treatmentText.textContent='—'); alertsBar && (alertsBar.textContent='No alerts'); return; }
    const last = n-1;
    const ph = chartSeries.ph[last], tds = chartSeries.tds[last], turb = chartSeries.turb[last], iron = chartSeries.iron[last];
    const thr = readThresholds();
    const alerts=[];
    if(ph!=null && (ph < thr.phLow || ph > thr.phHigh)) alerts.push({k:'ph',v:ph});
    if(tds!=null && tds > thr.tds) alerts.push({k:'tds',v:tds});
    if(turb!=null && turb > thr.turb) alerts.push({k:'turb',v:turb});
    if(iron!=null && iron > thr.iron) alerts.push({k:'iron',v:iron});

    function linearPredict(arr){ const x=[], y=[]; for(let i=0;i<arr.length;i++) if(arr[i]!=null){ x.push(i); y.push(arr[i]); } if(y.length<2) return null; const n=y.length; const sumx=x.reduce((a,b)=>a+b,0); const sumy=y.reduce((a,b)=>a+b,0); const sumxy = x.reduce((a,b,i)=>a+b*y[i],0); const sumxx = x.reduce((a,b)=>a+b*b,0); const denom = n*sumxx - sumx*sumx; if(denom===0) return null; const m=(n*sumxy - sumx*sumy)/denom; const c=(sumy - m*sumx)/n; return m*(x.length)+c; }
    const predWindow=3; const pred={};
    ['ph','tds','turb','iron'].forEach(key=>{ const arr = chartSeries[key].slice(-predWindow).map(v=>v==null?null:Number(v)); pred[key] = linearPredict(arr); });
    const predAlerts=[];
    if(pred.ph!=null && (pred.ph < thr.phLow || pred.ph > thr.phHigh)) predAlerts.push({k:'ph',v:pred.ph});
    if(pred.tds!=null && pred.tds > thr.tds) predAlerts.push({k:'tds',v:pred.tds});
    if(pred.turb!=null && pred.turb > thr.turb) predAlerts.push({k:'turb',v:pred.turb});
    if(pred.iron!=null && pred.iron > thr.iron) predAlerts.push({k:'iron',v:pred.iron});

    const top = alerts.length ? alerts[0] : (predAlerts.length ? predAlerts[0] : null);
    if(top){
      detectedText && (detectedText.textContent = top.k==='ph'?`pH ${formatNumber(top.v)}`:`${top.k.toUpperCase()} ${formatNumber(top.v)}`);
      ...
      alertsBar && (alertsBar.textContent = `${alerts.length} immediate • ${predAlerts.length} predicted alerts`);
      alertsBar && (alertsBar.style.background = 'linear-gradient(90deg, rgba(255,90,90,0.08), rgba(255,0,0,0.05))');
    } else {
      detectedText && (detectedText.textContent='No immediate issues'); treatmentText && (treatmentText.textContent='—'); alertsBar && (alertsBar.textContent='All good'); alertsBar && (alertsBar.style.background = 'linear-gradient(90deg, rgba(0,255,170,0.03), rgba(0,255,170,0.01))');
    }
  }
  function formatNumber(v){ if(v==null) return '—'; if(Math.abs(v)>1000) return Math.round(v); if(Math.abs(v)<1) return v.toFixed(2); return Number(v).toFixed(2); }

  // SSE (single instance, reconnect with backoff)
  let es = null;
  ...
  function startSSE(){
    ...
    try {
      ...
      es.onerror = (e) => {
        ...
        if(!sseClosedByUser){
          setTimeout(()=> {
            reconnectDelay = Math.min(reconnectDelay * 1.8, 30000);
            startSSE();
          }, reconnectDelay);
        }
      };
  ...
  analyzeBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if(json){
        if(json.generated_text) treatmentText.textContent = json.generated_text;
        else treatmentText.textContent = JSON.stringify(json, null, 2);
  ...
  // expose for debug
  window.__WAM = { rawData, chartSeries, feedRow, resetPlayback, startPlay, stopPlay };
